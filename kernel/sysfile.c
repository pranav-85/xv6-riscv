...
int sys_rename(void) {
  ...
  int isdir = old_ip->type == T_DIR;

  // Cannot rename "." or ".."
  if(isdir && (namecmp(oldname, ".") == 0 || namecmp(oldname, "..") == 0)) {
    iput(old_dp);
    iput(dp);
    iunlockput(old_ip);
    end_op();
  ...
  // Lock parent directories in order
  if (old_dp < dp) {
    ilock(old_dp);
    ilock(dp);
  } else if (old_dp > dp) {
  ...
  if(old_dp == dp){
    // Special case: rename within same directory
...
sys_createfile(void) {
    ...
    // Fetch the file path from the user space
    if (argstr(0, path, MAXPATH) < 0)
        return -1;
